...
async function getLaptops(page = 1) {
  try {
    ...
    $('.thumbnail').each((_i, elem) => {
      ...
      const rating = $(elem).find('.ratings p[data-rating]').attr('data-rating');
      const img = $(elem).find('img').attr('src')
      
      laptops.push({ title, titleLink, price, description, rating, reviews, img });
    });
...
async function getAllLaptops() {
  ...
  const filterLenovo = allLaptops.filter(laptop => laptop.title.toLowerCase().includes('lenovo'));
  
  filterLenovo.sort((a, b) => parseFloat(a.price.replace('$', '')) - parseFloat(b.price.replace('$', '')));
  return filterLenovo;
}
